    ...
    return (
      <div className="detail-data container-inner">
        {!detail_data.displayForm && (
          ...
          <a href="#" onClick={this.handleCallBack} className="back-link">
            <img style={{ marginRight: '8px' }} src={Back}></img>Back
          </a>
        )}
        {detail_data.displayForm ? (
          <div className="steps--main step-one">
            <span className="step--left active">01</span>
            <span className="step--right">/2</span>
          </div>
        ) : (
          <div className="steps--main step-two">
            <span className="step--left active">02</span>
            <span className="step--right">/2</span>
          </div>
        )}
        {detail_data.displayForm ? (
          <DetailsForm
            response={image.response_success && image.response_success}
            language_text={language_text}
            fields={fields}
            id={match.params.id}
            addDetail={data => this.props.addDetail(data)}
          />
        ) : (
